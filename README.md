...
- 本地草稿
  ...
  - 支援 JSON 匯入 / 匯出
- 管理員解鎖
  - 先用管理員 secret / 相容 write token 換短期 admin session
  - 前端只把 session 暫存在 `sessionStorage`
  - 關閉分頁後失效
- 正式後台資料
  - 透過正式 exec URL 讀寫同一份 runtime config
  - 解鎖後可載入正式資料覆蓋草稿
  - 可把目前草稿儲存成正式內容
  - 可透過 Cloudinary signed upload 上傳正式圖片並寫回 `photo.src` / `seo.ogImage`
...
- 不要把真正 write secret / Cloudinary secret 放在前端 env
- `/admin/` 只保存短期 admin session 到 `sessionStorage`
- 建議的 Script Properties：
  - `ADMIN_WRITE_SECRET` 或相容沿用 `CARD_ADMIN_WRITE_TOKEN`
