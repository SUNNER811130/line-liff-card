...
- live `initBackend`：
  - `Illegal spreadsheet id or key: 1evhAzJ3lmip0Aaiy5d0pd8pXc9-uP2zsDqOqBPq5Flg`
- `debugOpenSheet()` 已補回 repo 並 push 到正式 GAS：
  - 會輸出 `rawId` / `JSON.stringify(rawId)` / `rawIdLength`
  - 會輸出 `trimmedId` / `trimmedIdLength`
  - 會輸出 `cleanId` / `cleanIdLength`
  - 會檢查 `sheetName`
  - 會跑 `DriveApp.getFileById(cleanId)`
  - 會跑 `SpreadsheetApp.openById(cleanId)`
  - 會檢查 `getSheetByName(sheetName)`
- 但 `clasp run debugOpenSheet` 目前仍直接被 Google 拒絕：
...
- 下個接手者不要再重新建立 GAS 專案；請直接在現有正式 Apps Script 專案內確認：
  ...
  - 同一帳號在 Apps Script UI 內手動重新授權一次，至少執行 `debugOpenSheet()`，確認新的 Spreadsheet + Drive scope 已生效
