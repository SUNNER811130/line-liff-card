...
- live `initBackend`：
  - `Illegal spreadsheet id or key: 1evhAzJ3lmip0Aaiy5d0pd8pXc9-uP2zsDqOqBPq5Flg`
- `debugOpenSheet()` 已驗證 raw sheet id 格式正常：
  - `rawId` / `trimmedId` / `cleanId` 都正常
  - length = `44`
- 真正失敗點已縮到 `DriveApp.getFileById(cleanId)`，所以目前 blocker 更像是正式 GAS OAuth scopes / Spreadsheet authorization 沒完整覆蓋，而不是 sheet id 含 hidden char
...
- 下個接手者不要再重新建立 GAS 專案；請直接在現有正式 Apps Script 專案內確認：
  ...
  - 同一帳號在 Apps Script UI 內手動重新授權一次，至少執行會碰 `SpreadsheetApp.openById()` / `DriveApp.getFileById()` 的函式，確認新的 Spreadsheet + Drive scope 已生效
